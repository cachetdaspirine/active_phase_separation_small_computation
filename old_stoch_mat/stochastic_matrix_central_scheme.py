...
def make_transition_matrix(Va,Vb,Vc, # potential
    ...
    for i in range(n):
        ...
        if i>1 and i <n-2:
            matrix[i,i] += -1/(2*dx**2) - kac(x,Aac) - kab(x,Aab)
            matrix[i,n+i] += kba(x)
            ...
            matrix[i,i] += -1/(4*dx**2) - kac(x,Aac) - kab(x,Aab)
            matrix[i,n+i] += kba(x)
            matrix[i,2*n+i] += kca(x)
            matrix[i,i+1] += dVxdx/(2*dx)
            #matrix[i,i-1] += -dVxmdx/(2*dx)
    ...
    for i in range(n,2*n):
        ...
        if i>n+1 and i<2*n-2:
            matrix[i,i] += -1/(2*dx**2) - kba(x) - kbc(x,Abc)
            ...
            matrix[i,i] += -1/(4*dx**2) - kba(x) - kbc(x,Abc)
            matrix[i,i-n] += kab(x,Aab)
            matrix[i,i+n] += kcb(x)
            matrix[i,i+1] += dVxdx/(2*dx)
            #matrix[i,i-1] += -dVxmdx/(2*dx)
    ...
    for i in range(2*n,3*n):
        ...
        if i>2*n+1 and i < 3*n -2:
            matrix[i,i] += -1/(2*dx**2) - kca(x) - kcb(x)
            ...
            matrix[i,i] += -1/(4*dx**2) - kca(x) - kcb(x)
            matrix[i,i-2*n] += kac(x,Aac)
            matrix[i,i-n] += kbc(x,Abc)
            matrix[i,i+1] += dVxdx/(2*dx)
            #matrix[i,i-1] += -dVxmdx/(2*dx)
